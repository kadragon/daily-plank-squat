...
test('safe-area header region shares app-shell background with root fallback color', async () => {
  const css = await readIndexCss()
  const appHeaderBlock = css.match(/\.app-header\s*\{([\s\S]*?)\}/)?.[1] ?? ''
  const htmlBodyBlock = css.match(/html,\s*body\s*\{([\s\S]*?)\}/)?.[1] ?? ''

  expect(appHeaderBlock).toContain('background: var(--app-shell-background);')
